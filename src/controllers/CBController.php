        ...
        if ($this->sub_module) {
            foreach ($this->sub_module as $s) {
                ...
                $addaction[] = [
                    'label' => $s['label'],
                    'icon' => $s['button_icon'],
                    'url' => CRUDBooster::adminPath($s['path']).'?return_url='.urlencode(Request::fullUrl()).'&parent_table='.$table_parent.'&parent_columns='.$s['parent_columns'].'&parent_columns_alias='.(isset($s['parent_columns_alias']) ? $s['parent_columns_alias'] : '').'&parent_id=['.(! isset($s['custom_parent_id']) ? "id" : $s['custom_parent_id']).']&foreign_key='.$s['foreign_key'].'&label='.urlencode($return_label),
                    'color' => $s['button_color'],
                    'showIf' => isset($s['showIf']) ? $s['showIf'] : '',
                ];
            }
        }
